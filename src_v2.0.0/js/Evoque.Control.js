...
Evoque.control = (function (self)
{
    ...
    self.rangeSelect = function (option, inputElement)
    {
        ...
        option = lexus(option);
        var input;
        if (lexus.checkType(inputElement) === type.eElement)
        {
            input = lexus(inputElement);
        }
        else
        {
            var id = option.getValueOfProperty('inputId', defaultOption_RangeSelect);
            if (lexus.isStringEmpty(id))
            {
                throw 'Parameter is error!';
            }
            input = lexus('input[id="' + id + '"]');
        }
        ...
        input.each(function () {
            var re = null;
            if (lexus(this).getAttr('type') == 'text')
            {
        ...
        {
            var div = lexus(document.createElement('div'));
            div.addClass('form-add-sub');
            var input1 = lexus(input);
            input1.addClass('num');
            ...
            var aDown = lexus(document.createElement('a'));
            aDown.addClass('sub');
            ...
            var aUp = lexus(document.createElement('a'));
            aUp.addClass('add');
            ...
            return {
                setVal: function (v, forceChange) {
                    ...
                    if (lexus.checkType(forceChange) !== type.eBoolean || !forceChange)
                    {
    ...
    self.rangeSelect2 = function (option, inputElement)
    {
        ...
        if (lexus.checkType(initVal) !== type.eNumber || initVal < min || initVal > max)
        {
            initVal = min;
        }
        ...
        input.each(function () {
            var re = null;
            if (lexus(this).getAttr('type') == 'hidden')
            {
                re = generateRange(this);
            }
            ret.push(re);
        ...
        function generateRange(input)
        {
            ...
            var displayDiv = lexus(document.createElement('div'));
    ...
    self.button = function (option, element)
    {
        ...
        option = lexus(option);
        var btn;
        if (lexus.checkType(element) === type.eElement)
        {
            btn = lexus(element);
        }
        else
        ...
        if (lexus.hasTouchEvent())
        {
            btn.addEventHandler('touchstart', function () {
                lexus(this).addClass(classDown);
            });
            btn.addEventHandler('touchend', function () {
                lexus(this).removeClass(classDown);
            });
        }
        else
        {
    ...
    self.sliderBar = function (option, divElement)
    {
        ...
        option = lexus(option);
        var divCollection;
        if (lexus.checkType(divElement) === type.eElement)
        {
            divCollection = lexus(divElement);
        }
        else
        {
    ...
    self.rate = function (option, element)
    {
        ...
        else if (lexus.checkType(pointLevels[0]) !== type.eNumber)
        {
            pointLevels = defaultOption_Rate.pointLevels;
        }
        ...
        if (!readonly)
        {
            ...
            var $hid = lexus(hid);
            hid.type = 'hidden';
            if (!lexus.isStringEmpty(bindField))
            {
                hid.id = bindField;
        ...
        function resetInitPoint(isInit)
        {
            ...
            else
            {
                ...
                if (currentPointValue !== '') {
                    if (lexus.checkType(onPointChanged) === type.eFunction) {
        ...
        function setPointIndex()
        {
            ...
            currentPointValue = this.innerText;
            $desSpan.html('');
            if (!lexus.isStringEmpty(description[clickIdx]))
            {
