...
Evoque.extend('calendarV4', (function (self) {
    var defaultOption = {
    ...
    self.create = function (option) {
        ...
        var onConfirm = option.getValueOfProperty('onConfirm', defaultOption);
        var onCancel = option.getValueOfProperty('onCancel', defaultOption);

        caller.each(function () {
            var thisCache = lexus(this).cache();
            if (!thisCache.containsKey('calendar_v2'))
            {
                thisCache.push('calendar_v2', new calendarClass(this, startDate, endDate, mode, pickMode, onRenderDateTd, onBeforeSelect, onSelected, onConfirm, onCancel, timezoneOffset));
            }
        });
    };
    ...
    self.adaptFrame = function (frameHeight) {
        var parameters = arguments;
        var caller = self.evoqueTarget;
        caller.each(function () {
            var thisCache = lexus(this).cache();
            if (thisCache.containsKey('calendar_v2')) {
                thisCache.get('calendar_v2').adaptFrame.apply(thisCache.get('calendar_v2'), parameters);
            }
        });
    };

    self.foldMonthSelector = function () {
        var parameters = arguments;
        var caller = self.evoqueTarget;
        caller.each(function () {
            var thisCache = lexus(this).cache();
            if (thisCache.containsKey('calendar_v2')) {
                thisCache.get('calendar_v2').foldMonthSelector.apply(thisCache.get('calendar_v2'), parameters);
            }
        });
    };
    ...
    function calendarClass(element, minDate, maxDate, mode, pickMode, onRenderDateTd, onBeforeSelect, onSelected, onConfirm, onCancel, timezoneOffset)
    {
        ...
        $btnConfirm.click(function () {
            var ret = onConfirm.call(window, { selectDateStart: _finalStart, selectDateEnd: _finalEnd });
            if (lexus.checkType(ret) === type.eBoolean && !ret) {
                return;
            }
        });
        ...
        return {
            adaptFrame: function (frameHeight) {
                var height = frameHeight - lexus('div.m-calendar-btn')[0].clientHeight - lexus('div.m-calendar-hd')[0].clientHeight - lexus('ul.m-calendar-weekday')[0].clientHeight;
